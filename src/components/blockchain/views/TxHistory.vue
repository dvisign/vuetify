<template>
  <v-app id="smartContract">
    <div class="panel-heading">
      <h3 class="panel-title">블록체인 이력조회</h3>
    </div>
    <v-container fluid>
      <v-row>
        <v-col>
    ...
    >
      ...
      <!-- 미사용으로 인해 주석처리함 (table 에 뿌려주는 데이터를 KAS 에서 조회하기로 변경됨) -->
      <!--
        <template v-slot:item.KAS="{ item }">
          <v-btn v-show="item.type=='FT'" color="primary" @click="getKasData(item)">KAS 조회</v-btn>
        </template>
        -->
...
export default {
  ...
  mounted() {
    ...
    this.tableHeaders = [
      ...
      { text: "별칭", align: "center", sortable: false, value: "name" },
      {
        text: "총 포인트 발행량",
        align: "center",
        sortable: false,
      ...
      // 미사용으로 인해 주석처리함 (table 에 뿌려주는 데이터를 KAS 에서 조회하기로 변경됨)
      // { text: '포인트 보유량 확인', align: 'center', sortable: false, value: 'KAS'},
      {
        text: "포인트 추가 발행",
        align: "center",
        sortable: false,
    ...
    for (var i=0; i < 24; i++){
      this.timeList.push(String(i).padStart(2, '0'));
    }
  },
  methods: {
    snackbarTest: function() {
      this.$root.snackbar.show({ message: "Ahoy there!" });
    },
    ...
    // 미사용으로 인해 주석처리함 (table 에 뿌려주는 데이터를 KAS 에서 조회하기로 변경됨)
    // getKasData: function(row) {
    //   const targetUri = "/admin/contract/info/kas";

    //   let requestData = {
    //     // address & contractKeyType
    //     address: row.address,
    //     contractKeyType: "address"
    //   };
    ...
    addContract: function() {
      ...
      if (requestData.type == "FT") {
        if (this.addContractParams["initialsupply"] == "") {
          ...
          this.$root.snackbar.show({
            message: "필수데이터 최초 발행 포인트 수량이 누락되었습니다",
            color: "fail"
          });
          return;
        } else {
      ...
      this.$root.confirmDialog
        .show({
          message:
            "추가하신 계약은 삭제하실 수 없습니다. 계약을 추가하시겠습니까?"
        })
        .then(isConfirm => {
          if (isConfirm) {
            axios
    ...
    mintToken: function() {
      ...
      if (
        ...
        this.$root.snackbar.show({
          message:
            "총 포인트 발행량은 999,999,999,999,999,999 를 초과할 수 없습니다",
          color: "fail"
        });
        return;
      }
    ...
    burnToken: function() {
      ...
      if (this.amountOfBurn > this.selectedRow["balance"]) {
        this.$root.snackbar.show({
          message: "포인트 소각량은 관리자 보유 포인트량을 초과할 수 없습니다",
          color: "fail"
        });
        return;
      }
